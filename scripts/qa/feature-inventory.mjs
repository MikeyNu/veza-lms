...
async function controllerOperations() {
  const files = await walk(join(repositoryRoot, "apps", "api", "src"), (path) =>
...
async function applicationPages(appRoot, application) {
  ...
  return files.map((file) => {
    ...
    const route =
      dynamicRoute(`/${routePart === "." ? "" : routePart}`).replace(
        /\/$/,
        "",
      ) || "/";
    return {
...
async function bffRoutes(appRoot, application) {
  ...
  for (const file of files) {
    ...
    invariant(
      methods.size > 0,
      `BFF route ${repoPath(file)} does not expose a supported HTTP method`,
    );
...
async function workerCapabilities() {
  ...
  return files.map((file) => ({
    ...
    id: `worker:${repoPath(file)
      .replace(/^apps\/worker\/src\//, "")
      .replace(/\.ts$/, "")}`,
    source: repoPath(file),
  }));
}
...
async function validateWorkerRuntime() {
  ...
  for (const [capability, token] of requiredWorkerRuntimeEvidence) {
    invariant(
      mainWithoutWhitespace.includes(token.replace(/\s+/g, "")),
      `Worker runtime is not wired for ${capability}`,
    );
...
async function validateMigrationOrder() {
  ...
  const migrations = (await readdir(directory))
    ...
    .sort();
  const ownership = migrations.indexOf(
    "0038_aaa_system_schedule_ownership.sql",
  );
  ...
  const schedules = migrations.indexOf(
    "0053_platform_schedule_reconciliation.sql",
  );
  invariant(
    ownership >= 0 && ownership < media,
    "System schedule ownership must precede historical system schedules",
  );
  invariant(
    exports >= 0 && exports < schedules,
    "Export schema must precede platform schedule reconciliation",
  );
...
async function validateCatalogue() {
  ...
  invariant(
    Array.isArray(catalogue.categories) && catalogue.categories.length >= 20,
    "Feature catalogue is unexpectedly small",
  );
  const ids = new Set();
  const pathChecks = [];
  const gaps = [];
  ...
  for (const category of catalogue.categories) {
    invariant(
      category.id && category.title && category.boundary,
      `Feature category is incomplete: ${JSON.stringify(category)}`,
    );
    invariant(
      Array.isArray(category.features) && category.features.length > 0,
      `Feature category ${category.id} is empty`,
    );
    invariant(
      Array.isArray(category.sources) && category.sources.length > 0,
      `Feature category ${category.id} has no implementation surface`,
    );
    invariant(
      Array.isArray(category.tests) && category.tests.length > 0,
      `Feature category ${category.id} has no verification owner`,
    );
    for (const path of [...category.sources, ...category.tests])
      pathChecks.push({ category: category.id, path });
    for (const feature of category.features) {
      featureCount += 1;
      invariant(!ids.has(feature.id), `Duplicate feature id: ${feature.id}`);
      invariant(
        feature.id.startsWith(`${category.id}.`),
        `Feature ${feature.id} is outside its category namespace`,
      );
      invariant(
        allowedStatuses.has(feature.status),
        `Feature ${feature.id} has unsupported status ${feature.status}`,
      );
      invariant(
        !/[—]/.test(`${feature.name}${feature.note ?? ""}`),
        `Feature ${feature.id} contains a prohibited em dash`,
      );
      if (feature.entryPoint !== undefined) {
        const application = feature.application ?? "web";
        invariant(
          feature.status === "implemented",
          `Gap feature ${feature.id} cannot declare an implemented browser entry point`,
        );
        invariant(
          allowedApplications.has(application),
          `Feature ${feature.id} declares unsupported application ${application}`,
        );
        invariant(
          typeof feature.entryPoint === "string" &&
            feature.entryPoint.startsWith("/"),
          `Feature ${feature.id} has invalid browser entry point`,
        );
        invariant(
          !feature.entryPoint.includes("?"),
          `Feature ${feature.id} entry point must not contain a query string`,
        );
        browserEntryPoints.push({
          featureId: feature.id,
          application,
          route: feature.entryPoint,
        });
      }
      if (feature.status === "gap")
        gaps.push({ category: category.id, ...feature });
      ids.add(feature.id);
    }
  }
  invariant(
    featureCount >= 516,
    `Feature catalogue contains only ${featureCount} capabilities`,
  );
  invariant(
    gaps.length === 0,
    `Feature catalogue contains unresolved gaps: ${gaps.map((gap) => gap.id).join(", ")}`,
  );
  for (const id of criticalFeatureIds)
    invariant(
      ids.has(id),
      `Critical feature is missing from the catalogue: ${id}`,
    );
  for (const item of pathChecks) {
    invariant(
      await exists(join(repositoryRoot, item.path)),
      `Feature category ${item.category} references missing path ${item.path}`,
    );
  }
...
function validateBrowserEntryPoints(entryPoints, webPages, controlPages) {
  ...
  for (const entryPoint of entryPoints) {
    ...
    invariant(
      routes?.has(entryPoint.route),
      `Feature ${entryPoint.featureId} declares missing ${entryPoint.application} browser entry point ${entryPoint.route}`,
    );
...
async function main() {
  const [
    { featureCount, gaps, browserEntryPoints },
  ...
  const [apiOperations, webPages, controlPages, webBff, controlBff, workers] =
    await Promise.all([
      controllerOperations(),
      applicationPages(join(repositoryRoot, "apps", "web", "app"), "web"),
      applicationPages(
        join(repositoryRoot, "apps", "control-plane", "app"),
        "control-plane",
      ),
  ...
  const validatedBrowserEntryPoints = validateBrowserEntryPoints(
    browserEntryPoints,
    webPages,
    controlPages,
  );
  ...
  for (const surface of surfaces) {
    invariant(
      ...
      `Duplicate discovered surface: ${surface.id}`,
    );
    surfaceIds.add(surface.id);
  }
  invariant(
    apiOperations.length >= 100,
    `Only ${apiOperations.length} API operations were discovered`,
  );
  invariant(
    webPages.length >= 20,
    `Only ${webPages.length} web pages were discovered`,
  );
  invariant(
    controlPages.length >= 8,
    `Only ${controlPages.length} control-plane pages were discovered`,
  );
  invariant(
    webBff.length >= 20,
    `Only ${webBff.length} web BFF operations were discovered`,
  );
  invariant(
    workers.length >= 10,
    `Only ${workers.length} worker capabilities were discovered`,
  );

  await mkdir(artifactRoot, { recursive: true });
  await writeFile(
    join(artifactRoot, "catalogue.json"),
    `${JSON.stringify(catalogue, null, 2)}\n`,
  );
  await writeFile(
    join(artifactRoot, "discovered-surfaces.json"),
  ...
  console.log(
    `Feature inventory validated: ${featureCount} capabilities, ${surfaces.length} implementation surfaces, ${validatedBrowserEntryPoints.length} browser entry points, ${workerRuntimeEvidence.length} wired worker capabilities, no declared gaps.`,
  );
}
