...
import { addProgrammeOutcomeRequirement } from "../../../../../../../src/server/catalogue-api";
...
export async function POST(
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { message: "Outcome-requirement request is invalid." },
        { status: 400, headers: noStore },
      );
    }
    const { institutionId: _, ...input } = body;
    return NextResponse.json(
      await addProgrammeOutcomeRequirement(institutionId, versionId, input),
      { headers: noStore },
    );
  } catch (error) {
    return NextResponse.json(
      {
        message:
          error instanceof Error
            ? error.message
            : "Programme outcome requirement could not be added.",
      },
      { status: 400, headers: noStore },
    );
  }
