...
import { approveCurriculumValidationPolicy } from "../../../../../../../src/server/catalogue-api";
...
export async function POST(
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { message: "Validation-policy approval request is invalid." },
        { status: 400, headers: noStore },
      );
    }
    const { institutionId: _, ...input } = body;
    return NextResponse.json(
      await approveCurriculumValidationPolicy(institutionId, policyId, input),
      { headers: noStore },
    );
  } catch (error) {
    return NextResponse.json(
      {
        message:
          error instanceof Error
            ? error.message
            : "Validation policy could not be approved.",
      },
      { status: 400, headers: noStore },
    );
  }
