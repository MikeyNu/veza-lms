...
import { createCurriculumValidationPolicy } from "../../../../../src/server/catalogue-api";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      await createCurriculumValidationPolicy(institutionId, input),
    ...
    return NextResponse.json(
      {
        message:
          error instanceof Error
            ? error.message
            : "Curriculum validation policy could not be created.",
      },
      { status: 400, headers: noStore },
    );
  }
