...
export async function POST(request: NextRequest, { params }: { params: Promise<{ operation: string }> }) {
  if (!isSameOriginRequest(request.url, request.headers.get("origin"))) {
    return NextResponse.json({ message: "Cross-origin learner changes are not allowed." }, { status: 403, headers: noStore });
  }
  try {
    const { operation } = await params;
    if (!operations.has(operation)) {
      return NextResponse.json({ message: "Learner operation is invalid." }, { status: 404, headers: noStore });
    }
    const body = requireRecord(await request.json(), "Learner operation request");
    const enrolmentId = typeof body.enrolmentId === "string" ? body.enrolmentId : "";
    if (!uuid.test(enrolmentId)) {
      return NextResponse.json({ message: "Enrolment identifier is invalid." }, { status: 400, headers: noStore });
    }
    const { enrolmentId: _enrolmentId, ...input } = body;
