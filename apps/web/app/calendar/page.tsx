        ...
        <div className="calendar-layout">
          ...
          <aside className="calendar-context" aria-label="Selected session details">
            {selected ? <>
              ...
              <section className="attendance-panel"><header><div><h2>Attendance</h2><span>Live</span></div><div><button type="button" disabled>Mark all</button><button type="button" disabled>QR Code</button></div></header><div className="attendance-body"><div className="attendance-summary"><h3>Attendance Summary</h3><div className="attendance-donut"><strong>42</strong><span>Total</span></div><ul><li className="present">Present <strong>34 (81%)</strong></li><li className="late">Late <strong>5 (12%)</strong></li><li className="absent">Absent <strong>3 (7%)</strong></li></ul></div><div className="attendance-learners"><h3>Learner Attendance</h3><table><tbody>{[["Aarav Mehta","Present"],["Neha Iyer","Late"],["Rohan Das","Present"],["Tanvi Kapoor","Absent"],["Vikram Singh","Present"]].map(([name,status]) => <tr key={name}><td><span className="learner-avatar">{name.split(" ").map((part) => part[0]).join("")}</span>{name}</td><td><span className={`attendance-status ${status.toLowerCase()}`}>{status}</span></td></tr>)}</tbody></table><button className="attendance-view-all" type="button">View all 42 learners</button></div></div></section>
