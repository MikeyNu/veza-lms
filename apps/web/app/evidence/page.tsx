...
const evidenceRoles: readonly BaselineRoleKey[] = ["tenant-owner", "institution-admin", "registrar", "auditor"];
...
function filters(query: Query): AuditFilters {
  const eventType = single(query.eventType); const resourceType = single(query.resourceType); const resourceId = single(query.resourceId); const actorId = single(query.actorId); const from = single(query.from); const to = single(query.to); const cursor = single(query.cursor);
  return { ...(eventType ? { eventType } : {}), ...(resourceType ? { resourceType } : {}), ...(resourceId ? { resourceId } : {}), ...(actorId ? { actorId } : {}), ...(from ? { from } : {}), ...(to ? { to } : {}), ...(cursor ? { cursor } : {}), limit: 30 };
}
...
export default async function EvidencePage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  const [auditPage, workspace, catalogue, references] = await Promise.all([
    loadAuditEvents(selectedFilters), loadAcademicEvidenceWorkspace(institutionId), loadCatalogue(institutionId), loadCatalogueReferences(institutionId),
  ]);
  return <AppShell session={resolution.session} active="evidence"><EvidenceWorkspace institutionId={institutionId} workspace={workspace} catalogue={catalogue} references={references} /><EvidenceRoom page={auditPage} filters={selectedFilters} session={resolution.session}/></AppShell>;
}
