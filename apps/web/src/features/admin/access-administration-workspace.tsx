  ...
  return (
    <div className="access-workspace">
      <header className="access-heading">
        <div><p>IDENTITY & ACCESS</p><h1>Membership and invitation directory</h1><span>Manage verified identities, delegated roles and invitation evidence without trusting browser-supplied tenant context.</span></div>
        <nav aria-label="Access directory views"><button type="button" className={tab === "memberships" ? "active" : ""} onClick={() => setTab("memberships")}>Memberships <b>{directory.memberships.length}</b></button><button type="button" className={tab === "invitations" ? "active" : ""} onClick={() => setTab("invitations")}>Invitations <b>{directory.invitations.length}</b></button></nav>
      </header>
      ...
      {tab === "memberships" ? (
        <section className="access-layout">
          <div className="access-directory">
            <header><div><p>MEMBERSHIP REGISTER</p><h2>Verified identities</h2></div><span>{directory.memberships.length} shown</span></header>
            {directory.memberships.length ? (
              <div className="access-table-wrap"><table><thead><tr><th>Identity</th><th>Status</th><th>Roles</th><th>Created</th></tr></thead><tbody>{directory.memberships.map((membership) => (
                <tr key={membership.id} className={membership.id === selectedMembershipId ? "selected" : ""}>
                  <td><button className="access-member-select" type="button" onClick={() => setSelectedMembershipId(membership.id)} aria-pressed={membership.id === selectedMembershipId}><strong>{membership.identity.displayName ?? "Verified identity"}</strong><small>{membership.identity.email ?? membership.userId}</small></button></td>
                  <td><span className={`access-status ${membership.status}`}>{human(membership.status)}</span></td><td>{membership.roles.length}</td><td>{date(membership.createdAt)}</td>
                </tr>
              ))}</tbody></table></div>
