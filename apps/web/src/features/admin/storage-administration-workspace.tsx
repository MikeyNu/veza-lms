...
export function StorageAdministrationWorkspace({
  ...
  const overlayError = error ? <p className="admin-feedback error storage-overlay-error" role="alert">{error}</p> : null;
  ...
  return (
    <section className="storage-administration" aria-labelledby="storage-title">
      ...
      {!panel && !confirmation && error ? <p className="admin-feedback error" role="alert">{error}</p> : null}
      ...
      <section className="storage-config-grid storage-config-grid--records">
        <div className="storage-config-panel">
          <header><div><p className="admin-eyebrow">OBJECT BOUNDARY</p><h2>Namespaces</h2></div><div className="storage-panel-header-actions"><span>{workspace.namespaces.length}</span><Button type="button" size="small" variant="secondary" onClick={() => openPanel("namespace")}>New namespace</Button></div></header>
        ...
        <div className="storage-config-panel">
          <header><div><p className="admin-eyebrow">UPLOAD GOVERNANCE</p><h2>Storage policies</h2></div><div className="storage-panel-header-actions"><span>{workspace.policies.length}</span><Button type="button" size="small" variant="secondary" onClick={() => openPanel("policy")}>New policy</Button></div></header>
        ...
        <div className="storage-config-panel quota-panel">
          <header><div><p className="admin-eyebrow">CAPACITY POLICY</p><h2>Quota and cost guardrails</h2></div><div className="storage-panel-header-actions"><Status value={text(quota, "enforcement") || "unconfigured"}/><Button type="button" size="small" variant="secondary" onClick={() => openPanel("quota")}>Edit quota</Button></div></header>
          ...
          {selectedAsset ? (
            <>
              ...
              <section className="storage-inspector-actions">
                ...
                <Button type="button" size="small" variant="secondary" onClick={() => openPanel("accessibility")}>Record accessibility</Button>
              </section>
              <div className="storage-danger-zone">
                ...
                <Button type="button" size="small" variant="danger" disabled={busy?.startsWith("delete:") || bool(selectedAsset, "legal_hold", "legalHold")} onClick={() => openConfirmation({ type: "request-deletion", target: selectedAsset })}>Request deletion</Button>
              </div>
            </>
      ...
      <section className="storage-governance-grid">
        <div className="storage-governance-panel">
          <header><div><p className="admin-eyebrow">RECORDING CONSENT</p><h2>Consent register</h2></div><div className="storage-panel-header-actions"><span>{workspace.recordingConsents.length}</span><Button type="button" size="small" variant="secondary" onClick={() => openPanel("consent")}>Record consent</Button></div></header>
          <div className="storage-governance-list">{workspace.recordingConsents.map((consent) => (
            <article key={text(consent, "id")}>
              ...
              <div><Status value={text(consent, "state")}/>{text(consent, "state") === "granted" ? <Button type="button" size="small" variant="quiet" onClick={() => openConfirmation({ type: "withdraw-consent", target: consent })}>Withdraw</Button> : null}</div>
            </article>
          ))}</div>
        </div>
        ...
        <div className="storage-governance-panel">
          ...
          <div className="storage-governance-list deletion-list">{workspace.deletionRequests.length ? workspace.deletionRequests.map((request) => (
            <article key={text(request, "id")}>
              ...
              <div><Status value={text(request, "status")}/><small>Execute after {formatDate(field(request, "executeAfter", "execute_after"))}</small>{text(request, "status") === "requested" ? <Button type="button" size="small" variant="danger" onClick={() => openConfirmation({ type: "approve-deletion", target: request })}>Approve deletion</Button> : null}</div>
            </article>
      ...
      <Dialog
        ...
        onClose={() => { if (!busy) { setError(undefined); setConfirmation(null); } }}
        title={confirmationTitle}
        description={confirmationDescription}
        size="small"
        destructive={confirmation !== null}
        footer={
          <>
            <Button type="button" variant="secondary" onClick={() => { setError(undefined); setConfirmation(null); }} disabled={Boolean(busy)}>Cancel</Button>
            <Button type="submit" form="storage-confirmation-form" variant="danger" loading={Boolean(busy)} disabled={Boolean(busy)}>{confirmationLabel}</Button>
          </>
        }
      >
