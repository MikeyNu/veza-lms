...
export function ServiceHealth({ snapshot }: { snapshot: ServiceHealthSnapshot }) {
  ...
  return <section className="service-health" aria-labelledby="service-health-title">
    <header className="service-health-heading"><div><p className="cp-eyebrow">SERVICE OPERATIONS</p><h1 id="service-health-title">Platform readiness</h1><p>Deployment-facing health based on live database connectivity and transactional event backlog.</p></div><Link href="/health" className="health-refresh">Refresh snapshot <span aria-hidden="true">↻</span></Link></header>
    ...
    <section className="health-runbook"><div><p className="cp-eyebrow">OPERATOR RESPONSE</p><h2>Readiness interpretation</h2></div><ol><li><strong>Not ready</strong><span>Remove the instance from traffic and restore database connectivity before retrying.</span></li><li><strong>Degraded</strong><span>Keep core traffic available, inspect event workers and reconcile the outbox backlog.</span></li><li><strong>Ready</strong><span>Continue normal operations and monitor trends rather than a single snapshot.</span></li></ol></section>
  </section>;
}
