  ...
  return <form className="provision-layout" onSubmit={submit}>
    ...
    <section className="provision-canvas">
      ...
      {step === 1 ? <div className="form-grid">
        <label className="wide">Institution display name<input required maxLength={120} value={state.displayName} onChange={(event: ChangeEvent<HTMLInputElement>) => update("displayName", event.target.value)} placeholder="e.g. Sgela Academy"/></label>
        ...
        <label>Workspace slug<input required pattern="[a-z0-9](?:[a-z0-9-]{1,61}[a-z0-9])?" value={state.slug} onChange={(event: ChangeEvent<HTMLInputElement>) => update("slug", event.target.value.toLowerCase())} placeholder="sgela-academy"/><small>Used for domains, storage namespaces and support references.</small></label>
