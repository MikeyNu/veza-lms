  ...
  async createBlueprint(institutionId: string, input: CreateBlueprintDto) {
    ...
    return this.database.withTenantTransaction(context.tenantId, async (client) => {
      ...
      if (input.parentDefinitionId) {
        const parent = await client.query<{ definition_type: string }>(
        ...
        if (!parent.rows[0]) throw new NotFoundException("Parent curriculum definition was not found");
        const allowedParents: Readonly<Record<string, readonly string[]>> = {
          subject: [],
          module: ["subject"],
          course: ["subject", "module"],
          unit: ["subject", "module", "course"],
        };
        if (!allowedParents[input.definitionType].includes(parent.rows[0].definition_type)) {
          throw new BadRequestException(
            `${input.definitionType} cannot be nested under ${parent.rows[0].definition_type}`,
          );
        }
      } else if (input.definitionType === "unit") {
        throw new BadRequestException("A unit requires a parent subject, module or course");
      }
      ...
      if (outcomes.rowCount !== uniqueOutcomes.length) {
        throw new BadRequestException("Every mapped outcome must be active in this institution");
      }
      await client.query(
