...
interface AssignmentRow extends QueryResultRow { readonly ring_key: string; readonly version: number }
export interface TenantRingMutationResponse { readonly tenantId: string; readonly ringKey: string; readonly version: number }
...
export class TenantRingAssignmentService {
  ...
  async execute(principal: AuthenticatedPrincipal, tenantId: string, input: AssignTenantReleaseRingDto, idempotencyKey: string, correlationId: string): Promise<TenantRingMutationResponse> {
    ...
    return this.operations.run(
      ...
      canonicalHash({ tenantId, ringKey, expectedVersion: input.expectedVersion, reason }),
      async (client) => {
        ...
        const current = (await client.query<AssignmentRow>(`SELECT ring_key, version FROM tenant_release_assignments WHERE tenant_id = $1 FOR UPDATE`, [tenantId])).rows[0];
        ...
        const row = (current
          ? await client.query<AssignmentRow>(
              `UPDATE tenant_release_assignments
               SET ring_key = $2, reason = $3, version = version + 1,
                   assigned_by = $4, assigned_at = now(), updated_at = now()
               WHERE tenant_id = $1 AND version = $5 RETURNING ring_key, version`,
              [tenantId, ringKey, reason, principal.userId, input.expectedVersion],
            )
          : await client.query<AssignmentRow>(
              `INSERT INTO tenant_release_assignments (tenant_id, ring_key, reason, assigned_by)
               VALUES ($1,$2,$3,$4) RETURNING ring_key, version`,
              [tenantId, ringKey, reason, principal.userId],
            )).rows[0];
        ...
        await this.tenantAudit.append(client, {
          ...
          beforeState: { ringKey: current?.ring_key ?? "general-availability", version: current?.version ?? 0 },
          afterState: { ringKey: row.ring_key, version: row.version },
        });
        await this.platformAudit.append(client, {
          ...
          metadata: {
            ...
            ringKey: row.ring_key, previousVersion: current?.version ?? 0, version: row.version, reason,
          },
        });
        return { tenantId: tenant.id, ringKey: row.ring_key, version: row.version };
      },
    );
  }
}
