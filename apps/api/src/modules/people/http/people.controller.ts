...
export class PeopleController {
  ...
  @Get(":personId")
  @RequiresTenantPermission(permissions.peopleRead)
  detail(@Param("personId", new ParseUUIDPipe()) personId: string) { return this.people.detail(personId); }

  @Post()
  @RequiresTenantPermission(permissions.peopleCreate)
  create(@Body() input: CreatePersonDto) { return this.people.create(input); }

  @Put(":personId")
  @RequiresTenantPermission(permissions.peopleUpdate)
  update(@Param("personId", new ParseUUIDPipe()) personId: string, @Body() input: UpdatePersonDto) { return this.people.update(personId, input); }

  @Put(":personId/institutions/:institutionId/learner-profile")
  @RequiresTenantPermission(permissions.learnerManage)
  learner(@Param("personId", new ParseUUIDPipe()) personId: string, @Param("institutionId", new ParseUUIDPipe()) institutionId: string, @Body() input: UpsertLearnerProfileDto) { return this.people.upsertLearner(personId, institutionId, input); }
  ...
  relationship(@Param("personId", new ParseUUIDPipe()) personId: string, @Body() input: CreateRelationshipDto) { return this.people.createRelationship(personId, null, input); }
  ...
  dryRun(@Body() input: StagePeopleImportDto) { return this.people.stageImport(input); }

  @Post("imports/:importId/commit")
  @RequiresTenantPermission(permissions.peopleImportManage)
  @UseGuards(MfaGuard)
  commit(@Param("importId", new ParseUUIDPipe()) importId: string, @Body() input: CommitPeopleImportDto) { return this.people.commitImport(importId, input); }
}
