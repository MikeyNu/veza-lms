  ...
  async readiness(): Promise<ReadinessResponse> {
    ...
    try {
      ...
      const result = await this.database.controlPlaneQuery<OutboxHealthRow>(
        `SELECT count(*) FILTER (WHERE published_at IS NULL)::int AS pending_events,
                COALESCE(
      ...
      const eventDeliveryStatus: ComponentState = pendingEvents > maximumPendingEvents || oldestPendingSeconds > maximumOldestSeconds
        ? "degraded"
        : "up";
      ...
      return {
        ...
        checks: {
          ...
          eventDelivery: { status: eventDeliveryStatus, pendingEvents, oldestPendingSeconds },
        },
      };
    } catch {
      throw new ServiceUnavailableException({
        ...
        checks: {
          ...
          eventDelivery: { status: "down", pendingEvents: 0, oldestPendingSeconds: 0 },
        },
