...
test.before(async () => {
  ...
  await withTenant(
    ...
    (client) =>
      client.query(
        `INSERT INTO institutions (id,tenant_id,code,display_name,institution_type,locale,timezone,status,created_by)
     VALUES ($1,$2,'OTHER','Other institution','college','en-ZA','Africa/Johannesburg','active',$3)`,
...
test("approved blueprints are immutable and can create bounded delivery runs", async () => {
  await withTenant(tenant, async (client) => {
    ...
    await client.query("SAVEPOINT expected_blueprint_immutability");
    await assert.rejects(
      client.query(
        "UPDATE course_blueprint_versions SET delivery_modes=ARRAY['online'] WHERE id=$1",
        [blueprint],
      ),
      /immutable/i,
    );
    await client.query("ROLLBACK TO SAVEPOINT expected_blueprint_immutability");
    await client.query("RELEASE SAVEPOINT expected_blueprint_immutability");
    await client.query(
    ...
    const visible = await client.query(
      "SELECT id FROM course_runs WHERE id=$1",
      [courseRun],
    );
...
test("catalogue and enrolment rows remain invisible to another tenant", async () => {
  await withTenant(otherTenant, async (client) => {
    const rows = await client.query(
      "SELECT id FROM learning_outcomes UNION ALL SELECT id FROM course_runs UNION ALL SELECT id FROM enrolments",
    );
