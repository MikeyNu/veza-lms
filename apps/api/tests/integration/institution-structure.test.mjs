...
const appPool = new Pool({ connectionString: required("DATABASE_URL"), max: 4, statement_timeout: 10_000 });
...
async function seedControlPlane() {
  await controlPool.query(
    ...
    [actorA, `owner-a-${runId}`, `owner-a-${runId}@example.invalid`, actorB, `owner-b-${runId}`, `owner-b-${runId}@example.invalid`],
  );
  await controlPool.query(
    `INSERT INTO tenants (
    ...
    [tenantA, `institution-a-${runId}`, actorA, tenantB, `institution-b-${runId}`, actorB],
  );
}
...
async function seedInstitutions() {
  await withTenant(tenantA, async (client) => {
    ...
    await client.query("UPDATE organisational_units SET parent_unit_id = $1 WHERE id = $2", [rootUnit, childUnit]);
    await client.query(
    ...
    await client.query("UPDATE academic_periods SET parent_period_id = $1 WHERE id = $2", [yearPeriod, termPeriod]);
  }, true);
...
test("institution tables fail closed without tenant context", async () => {
  const result = await appPool.query("SELECT id FROM institutions WHERE id = ANY($1::uuid[])", [[institutionA, institutionB]]);
  assert.equal(result.rowCount, 0);
});

test("institution reads are restricted to the selected tenant", async () => {
  await withTenant(tenantA, async (client) => {
    const result = await client.query("SELECT id FROM institutions WHERE id = ANY($1::uuid[]) ORDER BY id", [[institutionA, institutionB]]);
    assert.deepEqual(result.rows.map((row) => row.id), [institutionA]);
  });
});

test("cross-tenant institution writes are rejected by RLS", async () => {
  await assert.rejects(
    withTenant(tenantA, (client) => client.query(
      `INSERT INTO institutions (
...
test("organisational-unit cycles are rejected by the database", async () => {
  await assert.rejects(
    withTenant(tenantA, (client) => client.query("UPDATE organisational_units SET parent_unit_id = $1 WHERE id = $2", [childUnit, rootUnit])),
...
test("a parent period cannot be shrunk around an existing child", async () => {
  await assert.rejects(
    withTenant(tenantA, (client) => client.query("UPDATE academic_periods SET starts_on = '2027-02-01' WHERE id = $1", [yearPeriod])),
...
test("published academic-period structure is immutable", async () => {
  await withTenant(tenantA, async (client) => {
    ...
    await assert.rejects(
      client.query("UPDATE academic_periods SET ends_on = '2027-12-30' WHERE id = $1", [yearPeriod]),
...
test("approved policy content is immutable and only one current version may exist", async () => {
  await withTenant(tenantA, async (client) => {
    ...
    await assert.rejects(
      client.query("UPDATE institutional_policies SET content = '{\"version\":2}'::jsonb WHERE id = $1", [first]),
